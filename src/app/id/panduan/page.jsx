    ...
    >
      <div className="max-w-6xl mx-auto px-6 md:px-8">
        ...
        <header className="mb-10 md:mb-14">
          ...
          <p className={`${isDark ? "text-gray-400" : "text-gray-600"} max-w-2xl mt-4`}>
            Semua yang kamu butuhkan untuk membeli, menjual, dan membiayai properti — ringkas,
            jelas, dan siap dipraktikkan.
          </p>
        </header>

        {/* Tabs */}
        <nav className={`relative border-b mb-8 ${isDark ? "border-white/10" : "border-gray-200"}`}>
    ...
    <ul className="space-y-3">
      {items.map((it, i) => (
        <li key={i} className="flex gap-3">
          ...
          <span className={`${isDark ? "text-gray-200" : "text-gray-700"}`}>{it}</span>
        </li>
      ))}
    </ul>
  );
    ...
    <div className="grid md:grid-cols-3 gap-6">
      <Card className="md:col-span-2" isDark={isDark}>
        ...
        <div className="mt-5 flex gap-3">
          <CTA href="/listing" isDark={isDark}>Jelajahi Listing</CTA>
          <CTA href="/panduan#legal" outline isDark={isDark}>Pelajari Legalitas</CTA>
        </div>
      </Card>

      <Card isDark={isDark}>
        <h4 className="text-sm text-gray-400 mb-2">Tips Cepat</h4>
  ...
  return (
    <div className="grid md:grid-cols-3 gap-6">
      <Card className="md:col-span-2" isDark={isDark}>
        ...
        <ListCheck items={list} isDark={isDark} />
        <div className="mt-5">
          <CTA href="/iklan/pasang" isDark={isDark}>Pasang Iklan Sekarang</CTA>
        </div>
      </Card>
      <Card isDark={isDark}>
  ...
  return (
    <div className="grid md:grid-cols-3 gap-6">
      <Card className="md:col-span-2" isDark={isDark}>
        ...
        />
        <div className="mt-5 flex gap-3">
          <CTA href="/kpr" isDark={isDark}>Buka Kalkulator KPR</CTA>
          <CTA href="/contact" outline isDark={isDark}>Konsultasi KPR</CTA>
        </div>
      </Card>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {items.map((it, i) => (
            ...
            >
              ...
              <p className={`${isDark ? "text-gray-300" : "text-gray-700"} text-sm`}>{it.text}</p>
            </div>
          ))}
        </div>
        <div className="mt-5">
          <CTA href="/contact" isDark={isDark}>Butuh Bantuan Notaris</CTA>
        </div>
      </Card>
    </div>
  );
