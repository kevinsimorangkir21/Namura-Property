...
export default function PropertyDetailID({ params }) {
  ...
  const title = prop.title?.id || prop.title;
  const desc = prop.desc?.id || prop.desc;
  const type = prop.type?.id || prop.type;
  ...
  const relatedHybrid = useMemo(() => {
    ...
    const sameType = propertiesData.filter(
      (p) => p.id !== prop.id && (p.type?.id || p.type) === type
    );
        ...
        >
          ...
          <a
            href={`https://wa.me/${prop.consultant?.whatsapp?.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(
              `Halo, saya tertarik dengan properti: ${title} (${prop.price}).`
            )}`}
            target="_blank"
              ...
              >
                ...
                <div>
                  ...
                  <span className="font-medium">
                    {years} tahun ({years * 12} bulan)
                  </span>
                </div>
                <div>
                  {t.interestRate}:{" "}
                  <span className="font-medium">{interest}% / tahun</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <section>
          ...
          {relatedHybrid.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {relatedHybrid.map((p) => {
                const relatedTitle = p.title?.id || p.title;
                const relatedType = p.type?.id || p.type;
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-3 flex justify-end">
                        <Link href={`/id/listing/${p.id}-${slugify(relatedTitle)}`}>
