...
export default function PropertyDetailEN({ params }) {
  ...
  const prop = propertiesData.find(
    (p) => `${p.id}-${slugify(p.title.en)}` === slug
  );

  if (!prop) return notFound();

  const details = prop.details.en;

  return (
    <div className="pt-28 pb-24 px-6 max-w-6xl mx-auto text-white">
      {/* Title */}
      <h1 className="text-4xl font-black">{prop.title.en}</h1>
      <p className="mt-3 text-gray-300">{prop.desc.en}</p>
