...
/* -------------------- Page -------------------- */
export default function PropertyDetailEN({ params }) {
  ...
  const t = {
    ...
    noSimilar: "No similar properties available.",
    perMeter: "Per m²",
  ...
  const title = prop.title?.en || prop.title;
  const desc = prop.desc?.en || prop.desc;
  const type = prop.type?.en || prop.type;
  ...
  const [price, setPrice] = useState(parsePrice);
  ...
  const relatedHybrid = useMemo(() => {
    const sameTypeSameProv = propertiesData.filter(
      (p) =>
        p.id !== prop.id &&
        (p.type?.en || p.type) === type &&
        getProvince(p.location) === currentProvince
    );

    if (sameTypeSameProv.length > 0) return sameTypeSameProv.slice(0, 4);

    const sameType = propertiesData.filter(
      (p) => p.id !== prop.id && (p.type?.en || p.type) === type
    );

    if (sameType.length > 0) return sameType.slice(0, 4);

    return propertiesData.filter((p) => p.id !== prop.id).slice(0, 4);
  }, [prop.id, type, currentProvince]);
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        >
          <div>
            <div className="text-[#00ccb0] font-black text-2xl sm:text-3xl">
              {prop.price}
            </div>
          ...
          <a
            href={`https://wa.me/${prop.consultant?.whatsapp?.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(
              `Hello, I'm interested in the property: ${title} (${prop.price}).`
            )}`}
            target="_blank"
        ...
        <section className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            >
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ["Property Price (Rp)", price.toLocaleString("en-US"), (v) => setPrice(Number(v.replace(/[^\d]/g, "")))],
                  ["Down Payment (%)", dpPercent, (v) => setDpPercent(Number(v))],
                  ["Tenor (years)", years, (v) => setYears(Number(v))],
                  ["Interest Rate (%/year)", interest, (v) => setInterest(Number(v))],
            ...
            >
              ...
              <div className="text-2xl font-extrabold text-[#00ccb0] mt-1">
                {formatIDR(monthly)}
              </div>
              ...
              >
                ...
                <div>
                  {t.interestRate}:{" "}
                  <span className="font-medium">{interest}% / year</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <section>
          ...
          {relatedHybrid.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {relatedHybrid.map((p) => {
                const relatedTitle = p.title?.en || p.title;
                const relatedType = p.type?.en || p.type;
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      >
                        ...
                        >
                          <Home className="w-3 h-3" /> {relatedType}
                        </span>
                      ...
                      <div className="text-[#00ccb0] font-bold text-sm mt-1">
                        {p.price}
                      </div>

                      <div className="mt-3 flex justify-end">
                        <Link href={`/en/listing/${p.id}-${slugify(relatedTitle)}`}>
