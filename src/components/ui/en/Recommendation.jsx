...
export default function Recommendation() {
  ...
  useEffect(() => {
    ...
    // Let premium appear first, then others randomized
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-6 gap-4">
          <div>
            ...
            >
              Curated property picks based on your search preferences and related location trends.
            </p>
          </div>

          {/* Buttons */}
          <div className="flex items-center gap-3">
            <Link href="/en/listing">
              <button
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <div
                    ...
                    } font-semibold text-[14px] mb-1 line-clamp-2`}
                  >
                    {getLocalizedText(p?.title, "en")}
                  </div>

                  <div
                    className={`${
                      isDark ? "text-gray-400" : "text-gray-500"
                    } text-[12px] mb-3 line-clamp-1`}
                  >
                    {getLocalizedText(p?.address || p?.location, "en")}
                  </div>

                  {/* Specs */}
                  <div
                    className={`${
                      isDark ? "text-gray-400/85" : "text-gray-600"
                    } text-[12px] flex items-center gap-3`}
                  >
                    <span className="inline-flex items-center gap-1.5">
                      <BedDouble className="w-4 h-4" /> {normalize(detail.kamarTidur)}
                    </span>
                    <span className="inline-flex items-center gap-1.5">
                      <Bath className="w-4 h-4" /> {normalize(detail.kamarMandi)}
                    </span>
                    <span className="inline-flex items-center gap-1.5">
                      <Ruler className="w-4 h-4" /> {normalize(detail.luasTanah)}
                    </span>
                  </div>

                  {p?.slug?.en && (
                    <Link href={`/en/property-list/${p.slug.en}`}>
